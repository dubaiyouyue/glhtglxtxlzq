 ...
 */
//获取所有项目和进度列表
$xm_all=$MYSQL->_sql_select('*','xm','xm_zt=2 or xm_id!=0','id','1','999');
//dump($xm_all);

foreach($xm_all as $key=>$v){
    if(!$v['xm_id'] && ($v['xm_zt']==2)) {
...
$id=_get('id')+0;
if(($a=='ghl') && $id){
    $MYSQL->_sql_update('xm','xm_zt=1,zt_time=NOW(),hq_user=\''.$user_login_name.'\'','id='.$id,'99');
	sleep(2);
	$MYSQL->_sql_update('xm','xm_zt=1','xm_id='.$id.' and xm_zt=2','999');
    echo '<script language="javascript" type="text/javascript">alert(\'提交成功！\');window.top.location.href="./?v='.$sj_zzfd.'";</script>';
    exit;
}
